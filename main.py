...
import os, logging, sys, json, re, ssl
import pymysql
...
if GEMINI_API_KEY:
    try:
        ...
        logger.info("Gemini AI enabled (model: %s)", GEMINI_MODEL)
    except Exception as e:
        logger.warning("Gemini init failed, falling back to rule-based agent only: %s", e)
        gemini_client = None
else:
...
def ask_gemini(user_message, products, orders):
    ...
    try:
        catalog = ", ".join(
            f"{p.get('name')} (${p.get('price')}, stock {p.get('stock', 0)})" for p in products
        ) or "no products loaded"
        context = (
            "Tum ek retail store ke friendly AI shopping assistant ho. "
            "Sirf store, products, orders, billing, ya shopping se related baaton mein madad karo. "
            f"Available products: {catalog}. "
            f"Total open orders: {len(orders)}. "
            "Jawaab chhota, seedha aur helpful rakho."
        )
...
def get_conn():
    if not (MYSQL_HOST and MYSQL_USER and MYSQL_DATABASE):
        raise RuntimeError(
            "MYSQL_HOST / MYSQL_USER / MYSQL_DATABASE env vars set nahi hain. "
            "Vercel Project Settings > Environment Variables me ye add karo aur redeploy karo."
        )
    ssl_ctx = None
    if MYSQL_SSL:
...
class Table:
    ...
    def _where(self, filt):
        ...
        for k, v in filt.items():
            ...
            if isinstance(v, dict) and "$regex" in v:
                clauses.append(f"LOWER({k}) LIKE %s")
                params.append(f"%{str(v['$regex']).lower()}%")
            else:
    ...
    def find(self, filt=None, projection=None):
        where_sql, params = self._where(filt)
        conn = get_conn()
        try:
            with conn.cursor() as cur:
                cur.execute(f"SELECT * FROM {self.table_name}{where_sql}", params)
                rows = cur.fetchall()
        finally:
            conn.close()
        for r in rows:
    ...
    def insert_one(self, doc):
        doc = {k: v for k, v in (doc or {}).items() if k in self.columns}
        if not doc:
    ...
    def update_one(self, filt, update):
        ...
        where_sql, where_params = self._where(filt)
        conn = get_conn()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    f"UPDATE {self.table_name} SET {', '.join(set_clauses)}{where_sql} LIMIT 1"
                    if where_sql else f"UPDATE {self.table_name} SET {', '.join(set_clauses)}",
                    params + where_params,
                )
    ...
    def delete_one(self, filt):
        return self._delete(filt, limit1=True)

    def delete_many(self, filt):
        return self._delete(filt, limit1=False)

    def _delete(self, filt, limit1):
        where_sql, params = self._where(filt)
...
class Database:
    def __init__(self):
        self.products = Table("products", ["name", "price", "stock", "category"])
        self.orders = Table(
            "orders",
...
def agent(msg_original, products, orders, database):
    ...
    if re.search(r'restock|add stock|update stock', msg):
        ...
        matched_product = fuzzy_match(msg, products)
        if matched_product:
            database.products.update_one(
...
def root():
    return {"status": "running", "service": "Retail AI Agent", "author": "Abhinav Shukla"}, 200

@app.route('/health', methods=['GET'])
def health():
    try:
        get_db().command("ping")
        return {"status": "healthy", "mysql": "connected"}, 200
    except Exception as e:
        return {"status": "healthy", "mysql": "disconnected", "error": str(e)}, 200
...
def query_agent():
    try:
        ...
        database = get_db()
        products = list(database.products.find({}).limit(10))
        orders = list(database.orders.find({}).limit(20))
...
def get_products():
    try:
        products = list(get_db().products.find({}).limit(20))
...
def get_orders():
    try:
        orders = list(get_db().orders.find({}).limit(20))
